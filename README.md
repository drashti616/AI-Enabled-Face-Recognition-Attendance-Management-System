...
│   └── uploads/                            # Uploaded employee profile photos
├── css/
...
├── database sql/
│   └── office_attendance_empty.sql         # Empty database structure (fresh setup)
...
```bash
mysql -u root -p < "database sql/office_attendance_empty.sql"
```
...
### 1. Login as Admin
There is no UI to create an admin — you need to insert one directly into the database.

Run this in MySQL (replace with your own details):
```sql
USE office_attendance;

-- Generate a bcrypt hash first by running in terminal:
-- python -c "import bcrypt; print(bcrypt.hashpw(b'YourPassword', bcrypt.gensalt()).decode())"

INSERT INTO admins (username, password_hash, full_name, email)
VALUES ('your_username', 'paste_generated_hash_here', 'Your Name', 'your@email.com');
```
...
- Run: `mysql -u root -p < "database sql/office_attendance_empty.sql"`
...
See `database sql/office_attendance_empty.sql` for complete schema.
